    ...
    return MaterialApp(
      ...
      home: ScannerScreen(
          onLanguageChanged: (languageCode) {
            ...
            print(
                'DEBUG: Parent setState completed, _currentLanguage=$_currentLanguage');
          },
    ...
    _hentInfo(ean).then((info) {
      if (info.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Produktet ble ikke funnet i databasen.'),
            ...
            onAddItem: (itemName) {
              ...
              if (!list.any((item) => item.endsWith(itemName))) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('"$itemName" lagt til i $listToAddTo'),
        ...
        Future<void> runSearch() async {
          ...
          try {
            ...
            if (response.statusCode == 200) {
              ...
              searchResults.value = (data['products'] as List<dynamic>?) ?? [];
            } else {
              searchResults.value = [];
            }
          } catch (_) {
                ...
                SnackBar(
                  content: Text(
                    AppLocalizations.of(context)?.networkError ??
                        'Kunne ikke hente søkeresultat. Sjekk nettverk og prøv igjen.',
                  ),
                ),
              );
            }
